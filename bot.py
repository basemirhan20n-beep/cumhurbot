...
KOOP_KODU, KOOP_UCRET, KOOP_GUN, KOOP_ZAMAN = range(4)
...
async def mood_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    metin = "😊 *Bugün sunucunun ruh hali nasıl?*\n\nAşağıdan oyunu ver:"
    markup = InlineKeyboardMarkup(keyboard)

    if update.callback_query:
        await update.callback_query.message.reply_text(
            metin, parse_mode="Markdown", reply_markup=markup
        )
    else:
        await update.message.reply_text(
            metin, parse_mode="Markdown", reply_markup=markup
...
async def koop_kodu_al(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if aktif >= db.MAX_KOOP:
        await update.message.reply_text(
            ...
            f"Bir kişi en fazla *{db.MAX_KOOP} koopa* katılabilir.\n"
            f"Çıkmak için /cik komutunu kullan.",
            parse_mode="Markdown",
            reply_markup=ana_menu(),
        )
...
async def ucret_sec(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([
        InlineKeyboardButton("🔙 Geri",  callback_data="geri_ucret"),
        InlineKeyboardButton("❌ İptal", callback_data="iptal"),
    ])
...
async def gun_sec(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([
        InlineKeyboardButton("🔙 Geri",  callback_data="geri_gun"),
        InlineKeyboardButton("❌ İptal", callback_data="iptal"),
    ])
...
async def cik_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    db.koop_listeden_cikar(update.effective_user.id)
    await update.message.reply_text(
...
async def bekleyenler_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for combo, uyeler in gruplar.items():
        ...
        for u in uyeler:
            k = db.kullanici_getir(u["user_id"])
            mention = (
                f"@{k['username']}" if k and k["username"]
                else (k["full_name"] if k else str(u["user_id"]))
            )
            metin += f"    • {mention}\n"
        metin += "\n"
...
async def ekipler_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ekipler:
        ...
        if update.callback_query:
            await update.callback_query.answer()
            await update.callback_query.message.reply_text(metin, reply_markup=markup)
        else:
            await update.message.reply_text(metin, reply_markup=markup)
        return
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "mood_menu":
        ...
        await q.message.edit_text("Ana Menü 👇", reply_markup=ana_menu())
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            KOOP_KODU: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, koop_kodu_al)
            ],
            KOOP_UCRET: [
                CallbackQueryHandler(ucret_sec,   pattern=r"^ucret_"),
                CallbackQueryHandler(geri_ucret,  pattern="^geri_ucret$"),
            ],
            KOOP_GUN: [
                CallbackQueryHandler(gun_sec,     pattern=r"^gun_\d+$"),
                CallbackQueryHandler(geri_gun,    pattern="^geri_gun$"),
                CallbackQueryHandler(geri_ucret,  pattern="^geri_ucret$"),
            ],
            KOOP_ZAMAN: [
                CallbackQueryHandler(zaman_sec,   pattern=r"^zaman_\d+$"),
                CallbackQueryHandler(geri_gun,    pattern="^geri_gun$"),
            ],
        },
        fallbacks=[
    ...
    app.add_handler(ChatMemberHandler(uye_takip, ChatMemberHandler.CHAT_MEMBER))

    # ❌ Genel mesaj handler YOK — bot yalnızca komutlara cevap verir
